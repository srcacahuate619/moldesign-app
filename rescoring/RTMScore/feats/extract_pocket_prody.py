...
#
# Codigo de terceros (RTMScore, MIT) que importa los bindings de Python de Open
# Babel. Open Babel es GPL-2.0-only, incompatible con la AGPL-3.0-only de
# MolDesign: importarlo desde codigo distribuido convertiria las dos obras en
# una sola. Aqui es legitimo porque este ayudante de preparacion de bolsillos
# NO viaja en el instalador y NADA del runtime lo importa -`gnn_service` usa
# `RTMScore.feats.mol2graph_pyg` y `RTMScore.model.*`, y nada mas-.
#
# Lo excluye `scripts/bundle_helper.py` y lo comprueba sobre el bundle real
# `scripts/check_openbabel_boundary.py`. Ver `docs/79_ADR_FRONTERA_OPEN_BABEL.md`.
