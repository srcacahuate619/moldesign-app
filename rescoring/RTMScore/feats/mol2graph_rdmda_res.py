...
def _pad_positions(residues, max_natoms: int):
    """Pad residue atom positions to max_natoms with NaN, or truncate if larger."""
    padded = []
    for res in residues:
        n_atoms = len(res.atoms)
        pos = res.atoms.positions[:max_natoms]  # truncate if > max_natoms
        if n_atoms < max_natoms:
            # Pad with NaN to reach max_natoms
...
def prot_to_graph(prot, cutoff):
    ...
    # [FIX] Compute dynamic max atoms per residue — some targets (6X1A) have
    # residues with >24 atoms (cofactors, modified residues). Using the
    # hardcoded RES_MAX_NATOMS=24 would cause np.full(negative) errors.
    ...
    g.ndata["pos"] = th.tensor(_pad_positions(u.residues, max_natoms))
    return g
...
def load_mol(molpath, explicit_H=False, use_chirality=True):
	...
	if re.search(r'.pdb$', molpath):
		...
		raise OSError("only the molecule files with .pdb|.sdf|.mol2 are supported!")	
