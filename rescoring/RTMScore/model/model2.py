...
class RTMScore(nn.Module):
	...
	def forward(self, bgp, bgl):		
		...
		self.N_l = N_l
		self.N_p = N_p
		# [FIX] Dynamic max atoms per residue — the hardcoded 24 caused
		# negative dimension errors on targets with residues >24 atoms (e.g. 6X1A).
	...
	def compute_euclidean_distances_matrix(self, X, Y):
		# [FIX] Use dynamic max_prot_atoms instead of hardcoded 24
		...
		return th.nan_to_num((dists**0.5).view(self.B, self.N_l, -1, max_atoms), 10000).min(axis=-1)[0]
