...
# [FIX] DGL solo se usa en training. Importarlo lazy para no requerirlo en inferencia con PyG.
# import dgl  # movido a la funcion collate donde realmente se necesita
...
class Meter:
    ...
    def roc_auc_score(self, reduction='none'):
        ...
        def score(y_true, y_pred):
            if len(y_true.unique()) == 1:
                print(f'Warning: Only one class {y_true[0]} present in y_true for a task. '
    ...
    def pr_auc_score(self, reduction='none'):
        ...
        def score(y_true, y_pred):
            if len(y_true.unique()) == 1:
                print(f'Warning: Only one class {y_true[0]} present in y_true for a task. '
                      'PR AUC score is not defined in that case.')
                return None
            else:
	...
	with th.no_grad():
		for batch_id, batch_data in enumerate(data_loader):
			...
			if pred or atom_contribution or res_contribution:
				...
				if pred:
					probx = th.zeros(bgl.batch_size, device=prob.device).index_add_(0, batch, prob)
					probs.append(probx)
...
def collate(data):
	import dgl  # [FIX] Lazy import: DGL solo se necesita para training
