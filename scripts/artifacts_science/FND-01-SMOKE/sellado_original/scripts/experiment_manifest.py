...
Flujo de trabajo:
    ...
    4. seal     congela los SHA-256 de datasets/modelos/binarios (soporta el cegamiento de FND-05)
...
def _check_sealed_hashes(manifest, errors):
    ...
    base = _git_toplevel()
    for key in ("dataset_hashes", "model_hashes", "binary_hashes"):
...
def _run_init(args):
    ...
    if manifest is not None:
        _atomic_write(exp_dir / "README.md", _render_readme(manifest))
    else:
        print(
...
def _run_seal(args):
    ...
    collected = {"dataset_hashes": {}, "model_hashes": {}, "binary_hashes": {}}
    for flag, key in (
        ("dataset", "dataset_hashes"),
        ("models", "model_hashes"),
        ("binaries", "binary_hashes"),
    ):
        for raw_path in getattr(args, flag, None) or []:
            try:
    ...
    print(
        ...
        f"{len(collected['binary_hashes'])} binary file hash(es) recorded."
    )
    return 0
...
def _build_parser():
    ...
    parser_seal = subparsers.add_parser(
        "seal", help="registra SHA-256 de datasets/modelos/binarios y sella el manifest"
    )
    parser_seal.add_argument("experiment_id", metavar="EXPERIMENT_ID")
    parser_seal.add_argument(
        "--dataset", action="append", default=None, metavar="PATH",
        help="archivo o directorio de dataset (recursivo); repetible",
    )
    parser_seal.add_argument(
        "--models", action="append", default=None, metavar="PATH",
        help="archivo o directorio de modelos (recursivo); repetible",
    )
    parser_seal.add_argument(
        "--binaries", action="append", default=None, metavar="PATH",
