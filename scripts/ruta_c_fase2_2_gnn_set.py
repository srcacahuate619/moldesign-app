# -*- coding: utf-8 -*-
"""
ruta_c_fase2_2_gnn_set.py — Ruta C, Fase 2.2 (opcion B):
GNN set-level con atencion cross-pose (PoseSelectorSet).

Iteracion FINAL de la linea GNN de Ruta C (docs/42_RUTA_C_PROTOCOLO.md).
Diagnostico que motiva el diseno: los encoders per-pose (GNN v1 0.4894,
GNN v2 0.5106) no capturan la relatividad intra-complejo — v0.6 (0.6596)
gana porque hace EXPLICITOS los z-scores por complejo. La opcion B aprende
esa relatividad de forma NATIVA: el modelo ve TODAS las poses de un complejo
juntas (codigo poblacional C4) con atencion dentro del complejo.

Arquitectura (PoseSelectorSet):

  Encoder per-pose (mismo que v2, clases y pesos REUTILIZADOS):
    ProteinEncoder GAT-Ca 24->128 (4 cabezas, 2 capas) +
    LigandEncoder GIN 38->128 (3 capas) + CrossAttention (bias distancia
    3.5 A + tipo de residuo) + Set2Set x2 (4 pasos) — hidden 128, de
    rescoring/gnn_v2/models.py. Rama ECIF 152->64 (MLP ReLU) + contexto
    5-dim ([vina_raw, z_vina, pct_vina, z_cluster, pct_cluster]).
    -> embedding per-pose h_i (dim 256, dropout 0.3) via proyeccion
       Linear(581->256).
    Warm-start: TODOS los componentes per-pose se inicializan desde
    data/pose_selector_dataset/pose_selector_v2_best.pt (se reportan las
    claves coincidentes; se omiten las cabezas de v2: fusion/score/rmsd).

  Relatividad cross-pose (NUEVO, C4):
    Sobre las N poses del complejo: 2 capas de TransformerEncoder
    (nn.TransformerEncoderLayer, d_model 256, nhead 4, dropout 0.2) con
    mascara intra-complejo en bloques sobre el lote completo (poses de
    pids distintos jamas se atienden). TODAS las poses de un split se
    procesan en un solo forward del set-encoder; la GNN per-pose se
    bachea por memoria (lotes de 64) y luego se concatena.

    Despues de la atencion, por pose:
      [h'_i, mean(h')_complejo, max(h')_complejo, h_i - mean(h')_complejo]
      (4 x 256 = 1024) -> fusion MLP -> cabezas score (1) + rmsd aux (1).

  DESVIACIONES HONESTAS (por limite de parametros <= 1.6 M, ver caveats):
    - dim_feedforward = 128 (el default de TransformerEncoderLayer es 2048;
      con el backbone warm-start de ~613 K no hay presupuesto para mas).
    - Fusion de una sola capa Linear(1024->128) + ReLU + dropout 0.3
      (una fusion de dos capas 1024->256->128 excederia el limite).

Perdida (C3, igual que v2):
  pairwise por complejo con margen clip(0.3 + 0.5*(rmsd_j - rmsd_i),
  0.3, 2.0) dominante + MSE auxiliar sobre rmsd centrado por complejo
  (lambda 0.1). Se reportan ambas curvas (¿se movio la perdida pair?).

Entrenamiento: Adam lr 1e-3 (backbone) / 3e-3 (atencion+fusion+cabezas),
weight_decay 1e-5, semilla 42, <= 60 epocas, early stop en top-1 de val
(paciencia 12). GPU si hay. Checkpoint:
data/pose_selector_dataset/pose_selector_set_best.pt (estado + config +
metricas val). MC-dropout (dropout activo en inferencia, mc_samples=20).

Evaluacion: por pid argmax(score) -> top-1 crystal-like (<= 2.0 A),
mediana de RMSD, Spearman medio por pid — en val (por epoca) y test
(final). Tabla: Vina 0.5319 / v0.6 0.6596 / GNN v1 0.4894 / GNN v2 0.5106
/ B (set). Gate: B > 0.6596 -> H-RC vindicada; si no, se cierra la linea
GNN y v0.6 se promueve como selector de Ruta C.

Bonus interpretabilidad: peso medio de atencion que cada pose asigna a la
pose finalmente seleccionada (capa de atencion final, media sobre cabezas).

Artefacto: scripts/artifacts_ruta_c_fase2_2.json (escritura incremental).
Pipeline ECIF reutiliza data/pose_selector_dataset/ecif_152_progress.jsonl.
Entrenamiento reanudable: data/pose_selector_dataset/train_gnn_set_progress.json.

Uso:
  python scripts/ruta_c_fase2_2_gnn_set.py            # pipeline completo
  python scripts/ruta_c_fase2_2_gnn_set.py --smoke    # chequeo rapido sin entrenar
"""
...
CKPT_V2 = DATASET_DIR / "pose_selector_v2_best.pt"
CKPT_SET = DATASET_DIR / "pose_selector_set_best.pt"
ARTIFACTOS = PROJECT_ROOT / "scripts" / "artifacts_ruta_c_fase2_2.json"
ECIF_CACHE = DATASET_DIR / "ecif_152_progress.jsonl"
PROGRESO_ENTRENO = DATASET_DIR / "train_gnn_set_progress.json"
LOG_PATH = DATASET_DIR / "train_gnn_set.log"

SEMILLA = 42
HIDDEN = 128          # requerido por el warm-start de v2 (checkpoint hidden=128)
DROPOUT = 0.3
SET2SET_STEPS = 4
ECIF_DIM = 152
ECIF_EMB = 64
...
CLAVES_CABEZA_V2 = ("fusion.", "score_head.", "rmsd_head.")
...
class PoseSelectorSet(nn.Module):
    """GNN set-level (opcion B): encoder per-pose v2 (warm-start) +
    ...
    def __init__(self, prot_in: int = 24, lig_in: int = 38,
        ...
        # ── encoder per-pose (clases de v2; pesos warm-start) ──
...
def cargar_warm_start_v2(modelo: PoseSelectorSet) -> dict:
    """Inicializa los componentes per-pose desde pose_selector_v2_best.pt
    por coincidencia de nombre+forma. Las cabezas de v2 (fusion.0.*,
    score_head.*, rmsd_head.*) se omiten explicitamente."""
    ck = torch.load(CKPT_V2, map_location="cpu", weights_only=False)
    sd = ck["modelo"]
    ...
    for k, v in propio.items():
        if (k in sd and sd[k].shape == v.shape
                and not k.startswith(CLAVES_CABEZA_V2)):
            propio[k] = sd[k]
            coinciden.append(k)
        else:
            nuevas.append(k)
    for k in sd:
        if k not in propio or k.startswith(CLAVES_CABEZA_V2):
            omitidas.append(k)
    modelo.load_state_dict(propio)
    return {
        "fuente": str(CKPT_V2.relative_to(PROJECT_ROOT)),
        "epoch_v2": int(ck.get("epoch", -1)),
        "val_v2": dict(ck.get("val", {})),
        "total_claves_v2": len(sd),
        "coincidentes": len(coinciden),
        "omitidas_cabezas_v2": omitidas,
        "nuevas_aleatorias": nuevas,
        "mapeo": ("coincidencia por nombre de submodulo: prot_encoder.*, "
                  "lig_encoder.*, cross_attn.*, lig_pool.*, prot_pool.*, "
                  "ecif_encoder.* (64 claves esperadas); las cabezas de v2 "
                  "(fusion/score/rmsd) se omiten por diseno"),
...
def entrenar(train, val, pids_val, device, i_idx, j_idx, marg_dev,
    ...
    pr(f"  [set] GNN set-level (opcion B) | params: {n_params:,} "
...
def main() -> None:
    ...
    configurar_salida()
    t0 = time.monotonic()
    pr("== Ruta C Fase 2.2: GNN set-level con atencion cross-pose "
       "(opcion B, docs/42) ==")
    ...
    if args.smoke:
        ...
        modelo = PoseSelectorSet()
        rep = cargar_warm_start_v2(modelo)
        n_params = contar_params(modelo)
        pr(f"  smoke: params {n_params:,} | coinciden "
           f"{rep['coincidentes']}/{rep['total_claves_v2']} | omitidas "
           f"{len(rep['omitidas_cabezas_v2'])}")
        ch = datos["val"][:2]
        with torch.no_grad():
    ...
    art: dict = {
        ...
        "fase": ("2.2 opcion B (GNN set-level con atencion cross-pose, "
                 "iteracion FINAL de la linea GNN)"),
        "hipotesis": ("la relatividad intra-complejo se aprende de forma "
                      "NATIVA: el modelo ve TODAS las poses del complejo "
                      "juntas (codigo poblacional C4) con atencion "
                      "intra-complejo; el score por pose depende del "
                      "conjunto completo, superando al campeon v0.6 "
                      "(0.6596) en el holdout congelado"),
        "gate_criterio": f"pose_selector_set_test_top1 > {TOP1_V06_TEST} "
                         f"(top-1 v0.6)",
        "config": {
            "arquitectura": {
                "encoder_per_pose": ("ProteinEncoder GAT-Ca 24->128 (4 "
                                     ...
                                     "desde pose_selector_v2_best.pt"),
                "hidden_dim": HIDDEN,
                "ecif_rama": f"MLP {ECIF_DIM}->{ECIF_EMB}, ReLU (warm-start)",
    ...
    reporte_warm = cargar_warm_start_v2(modelo_sonda)
    del modelo_sonda
    pr(f"  warm-start v2: {reporte_warm['coincidentes']}/"
       f"{reporte_warm['total_claves_v2']} claves coinciden; "
       f"omitidas (cabezas v2): {len(reporte_warm['omitidas_cabezas_v2'])}; "
    ...
    art["entrenamiento_set"] = {
        "descripcion": ("GNN set-level (opcion B): encoder per-pose v2 "
                        "warm-start + atencion cross-pose C4"),
    ...
    for split in ("val", "test"):
        tabla[split] = {
            ...
            "gnn_set_B_top1": met[split]["top1_rate"],
            "gnn_set_B_mediana": met[split]["mediana_rmsd"],
            "gnn_set_B_spearman": met[split]["spearman_pred_vs_rmsd_media"],
        }
    ...
    if set_test > TOP1_V06_TEST:
        veredicto = ("PASS — B supera a v0.6: la hipotesis H-RC queda "
                     "vindicada y la linea GNN continua con la opcion B")
    else:
        veredicto = ("FAIL — B no supera a v0.6: se cierra honestamente la "
                     "linea GNN de Ruta C y v0.6 (XGBoost) se promueve "
                     "como selector de poses de Ruta C")
    art["gate"] = {
        "criterio": f"pose_selector_set_test_top1 > {TOP1_V06_TEST} "
                    f"(campeon v0.6)",
        "regla_de_decision": ("si B (set-level) test top-1 > 0.6596 -> "
                              "H-RC vindicada (continuar con B); si NO -> "
                              "la linea GNN se cierra honestamente y v0.6 "
                              "XGBoost se promueve como selector de "
                              "Ruta C (el orquestador aplica la regla)"),
        "B_top1_test": set_test,
    ...
    pr(f"  Gate: {art['gate']['resultado']} "
       f"(B {set_test} vs v0.6 {TOP1_V06_TEST})")
    ...
    art["caveats_honestos"] = [
        ("dim_feedforward del TransformerEncoderLayer = 128 (default "
         "2048): con el backbone warm-start de v2 (~613 K params), 2 capas "
        ...
        ("El warm-start proviene de pose_selector_v2_best.pt, cuyo "
         "backbone se entreno con CL sobre el dataset PDBbind docked del "
         "proyecto (708 complejos) y luego con ranking sobre las poses de "
         "Ruta C."),
